...
export class SlaveWorker extends EpDbWorker {
    ...
    huntForJobs() {
        this.db.findJobByStatusAndRun(JobStatusEnum.STORED).then((job: JobDBO) => {
            if (job && job.jobDefinitionId) {
                const config: JobConfig = { deleteWorkspaceOnError: false, name: 'default', recoverOnFail: false, deleteWorkspaceOnFinish: false, entryFile: 'index.js' };
    ...
    updateError(jobId: string, status: JobStatusEnum, config: JobConfig, workspaceDirectory: string, jobDefinitionId: string, error: any, jobResolved?: Job) {
        const up: JobDBO = {
            _id: jobId,
            config: config,
